...
bool VNDO(SolvObject* state, unsigned int initialNvndo, unsigned short nifvndo){
    ...
    unsigned int numberOfVariables = state->getListOfVariables()->size();
	 ...
	 // neighbourhood bound
    unsigned int nvndo = initialNvndo;
    ...
    while(true) {
        ...
        // get the best neighbour and continue
            ...
            flips = 1;
				...
				// reset neighbourhood bound to initial one
            nvndo = initialNvndo;
